...
public class Main {
	...
	public static void main(String[] args) throws ParseException {
		...
		try (JavaSparkContext sc = new JavaSparkContext(sparkConf)) {
			...
			// " LIMIT 15000"
			DataFrame[] credibleData = getBodyContent(sqlContxt, conf.getCredibleDataset(), "BodyText", "WHERE (BodyText IS NOT NULL AND BodyText <> '') LIMIT 25000",
			...
			VectorAssembler assembler = new VectorAssembler()
					  .setInputCols(new String[]{idf.getOutputCol(), word2Vec.getOutputCol(), "commonfeatures"})
...
//			StructField predictionColSchema = predictions.schema().apply("prediction");
			...
			System.out.println(model.bestModel().extractParamMap().toString());
		}
	}
